...
namespace Other
{
    internal class UpdateManager
    {
        ...
        public async Task CheckForUpdate(string currentVersion)
        {
            ...
            var (latestVersion, latestZipUrl) = await githubManager.GetLatestReleaseInfo("Babyhamsta", "A1mmy");
        ...
        {
            // Download the newest release of A1mmy to %temp%
            string envTempPath = Path.GetTempPath();
            string localZipPath = Path.Combine(envTempPath, "A1mmyUpdate.zip");
            ...
            string extractPath = Path.Combine(envTempPath, "A1mmyUpdate");
            await Task.Run(() => // Run extraction in a separate task
            {
                ZipFile.ExtractToDirectory(localZipPath, extractPath, true);
            });

            // Create a batch script to move the files and restart A1mmy
